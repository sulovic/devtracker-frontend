...
const Users: React.FC = () => {
  ...
  const fetchUsers: () => void = async () => {
    setShowSpinner(true);
    try {
      const response: { data: AuthUser[] } =
        await axiosPrivate.get("/api/users");
  ...
  const handleDeleteUserOK: () => void = async () => {
    try {
      ...
      toast.success(
        `Korisnik ${selectedUser?.firstName + " " + selectedUser?.lastName} je uspešno obrisan!`,
        {
          position: "top-center",
        },
      );
  ...
  return (
    <>
          ...
          <div className="table-responsive p-3">
            <table className="w-full text-center text-sm text-zinc-500 rtl:text-right dark:text-zinc-400">
              <thead className="bg-zinc-200 uppercase text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400">
                <tr>
                  <th className="w-2/12 px-4 py-4">Ime i prezime</th>
                  <th className="w-2/12 px-4 py-4"> Email</th>
                  <th className="w-6/12 px-4 py-4"> Nivo ovlašćenja</th>
                ...
                {usersData.length
                  ? usersData.map((user, index) => (
                      ...
                      >
                        ...
                        <td key={`role_${index}`}>
                          <div className="align-items-center flex justify-center gap-2">
                            {user?.roles &&
                              user?.roles.map((role) => {
                                return (
                                  <div key={role?.userRole?.roleId}>
      ...
      {selectedUser && showModalEditUser && (
        <ModalEditUser
